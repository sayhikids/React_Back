...
public class JwtAuthenicationFilter extends OncePerRequestFilter{
    ...
    private String parseBearerToken(HttpServletRequest request) {
        ...
        //bearerToken : 토큰을 전송하면 브라우저에 토큰의 키 이름을 나타낸다.
        //이 값이 있다면 토큰 값만 분리해서 리턴한다.
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            if (token != null && !token.equalsIgnoreCase("null")) {
                ...
                //인증이 완료되면 인증을 담당하는 ContextHolder에 등록을 해줘야만 boot Secure 에서 인증된 사용자로 인식한다.
                //UsernamePasswordAuthenticationToken 의 첫번째 파라미터인 principal(UserId)은 해당 ID로 부여된 토큰을 filter가 검증할 수 있는 key이다.
                //이 값은 주어진 변수 이름 그대로 컨트롤러에서 사용할 수 있는데 그 제공자로 @AuthXXXPrincipal 어노테이션을 제공한다.
